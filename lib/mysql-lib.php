...
function db_sysconnect()
{
	...
	$SYSCONN=mysqli_connect($db_syshost,$db_sysuname,$db_syspasswd,$db_sysdbname) or systemlog("SQL connection error: ".mysql_error());		//and die?? TODO
	return $SYSCONN;
}
function db_sysclose($SYSCONN)
{
	mysqli_close($SYSCONN) or systemlog("SQL connection error: ".mysql_error());
}

function db_admconnect()	//not required?
{
	global $db_admhost,$db_admuname,$db_admpasswd,$db_admdbname;
	$ADMCONN=mysqli_connect($db_admhost,$db_admuname,$db_admpasswd,$db_admdbname) or systemlog("SQL connection error: ".mysql_error()); //and die?? TODO
	return $ADMCONN;
}
function db_admclose($ADMCONN)	//not required?
{
	mysqli_close($ADMCONN) or systemlog("SQL connection error: ".mysql_error());
}
?>