...
<?php
	...
	$result=mysqli_query($SYSCONN,"SELECT * FROM posts WHERE postid='".$_GET['postid']."';") or systemlog($_SERVER['PHP_SELF']."  SQL query error: ".mysqli_error($SYSCONN));
	$postinfo=mysqli_fetch_array($result);
	$result=mysqli_query($SYSCONN,"SELECT fullname FROM users WHERE uid='".$postinfo['authoruid']."';") or systemlog($_SERVER['PHP_SELF']."  SQL query error: ".mysqli_error($SYSCONN));
	$result=mysqli_fetch_array($result);
	$authorname=$result['fullname'];
	$result=mysqli_query($SYSCONN,"SELECT content FROM postdata WHERE postid='".$_GET['postid']."';") or systemlog($_SERVER['PHP_SELF']."  SQL query error: ".mysqli_error($SYSCONN));
	$result=mysqli_fetch_array($result);
